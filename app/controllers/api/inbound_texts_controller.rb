class Api::InboundTextsController < ApplicationController
  ...
  def bp_create
    ...
    if blood_pressure_log.save
      text_body = "Hi #{user.first_name} you have successfully added a blood pressure reading of #{blood_pressure_log.systolic} / #{blood_pressure_log.diastolic} on #{blood_pressure_log.log_time.strftime('%A, %b %d')} at #{blood_pressure_log.log_time.strftime('%H:%M')}"
    else
